    ...
    <div className="max-w-screen-xl mx-auto md:w-5/6 lg:w-4/6 pt-10">
      ...
      <div className="w-full overflow-hidden mb-6 [mask-image:_linear-gradient(to_right,transparent_0,_black_80px,_black_calc(100%-80px),transparent_100%)]">
        <div className="flex animate-scroll">
          {/* Duplicamos visualmente los elementos para efecto infinito */}
          {[...tecnologiasData, ...tecnologiasData].map((tecnologia, index) => (
            <div key={index} className="flex flex-col items-center mx-3">
              <img
                className="w-10 h-10 filter grayscale pointer-events-none cursor-none"
                src={`${process.env.NEXT_PUBLIC_BASE_PATH || ""}/img/${tecnologia.imagen}.svg`}
                alt={`imagen de ${tecnologia.nombre}`}
              />
              <p className="text-sm md:text-base text-center font-semibold text-white">{tecnologia.nombre}</p>
            </div>
          ))}
        </div>
      </div>
